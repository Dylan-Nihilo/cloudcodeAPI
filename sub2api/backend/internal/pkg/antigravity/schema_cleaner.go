...
func cleanJSONSchemaRecursive(value any) any {
	...
	if looksLikeSchema {
		// Gemini rejects JSON Schema "const", but "enum" with a single value
		// preserves the same constraint semantics for tool arguments.
		if constVal, ok := schemaMap["const"]; ok {
			if _, exists := schemaMap["enum"]; !exists {
				schemaMap["enum"] = []any{deepCopy(constVal)}
			}
			if _, exists := schemaMap["type"]; !exists {
				schemaMap["type"] = inferSchemaType(constVal)
			}
		}
...
func inferSchemaType(val any) string {
	switch val.(type) {
	case string:
		return "string"
	case bool:
