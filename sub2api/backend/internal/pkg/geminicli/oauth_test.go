...
func setBuiltinGeminiOAuthEnv(t *testing.T) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, "test-gemini-cli-client-id")
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "test-gemini-cli-client-secret")
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) && !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("错误消息应提及内置 OAuth 环境变量，实际: %v", err)
	}
}
