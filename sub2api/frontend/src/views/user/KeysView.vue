<template>
  <AppLayout>
    <TablePageLayout>
      <!-- Page header — editorial section title + ghost actions -->
      <template #actions>
        <div class="flex items-end justify-between pb-1">
          <div>
            ...
            <h1 class="text-[24px] font-light tracking-[-0.015em] text-ink">
              {{ apiKeys?.length || 0 }} {{ t('common.total', 'in total') }}
            </h1>
          </div>
      ...
      <!-- Filter strip — flat row, no card wrapping, mono labels -->
      <template #filters>
        <div class="border-y border-border py-3 flex flex-col gap-3">
      ...
      <template #table>
        ...
        >
          <template #cell-key="{ value, row }">
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded-lg p-1 transition-colors hover:bg-ink/5"
                :class="
                  copiedKeyId === row.id
                    ? 'text-success'
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-ink/5"
              ...
              >
                ...
                <span class="text-xs text-muted">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-subtle opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-muted">{{ t('keys.today') }}:</span>
                <span class="font-medium text-ink">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-muted">{{ t('keys.total') }}:</span>
                <span class="font-medium text-ink">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-muted">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-ink/10">
                  <div
                    :class="[
          ...
          <template #cell-rate_limit="{ row }">
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-subtle tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-subtle tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-subtle tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-muted transition-colors hover:text-ink hover:bg-ink/5"
          ...
          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-danger' : 'text-muted'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-subtle">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-muted">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-subtle">-</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:text-success hover:bg-success/[0.06]"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:text-ink hover:bg-ink/5"
              >
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'text-muted hover:text-warning hover:bg-warning/[0.06]'
                    : 'text-muted hover:text-success hover:bg-success/[0.06]'
                ]"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:text-danger hover:bg-danger/[0.04]"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-subtle">{{ t('keys.selectGroup') }}</span>
            </template>
        ...
        <div v-if="!showEditModal" class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-ink' : 'bg-ink/10'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.use_custom_key">
            <input
              ...
              :class="{ '!border-danger/60': customKeyError }"
            />
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_ip_restriction ? 'bg-ink' : 'bg-ink/10'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_quota ? 'bg-ink' : 'bg-ink/10'
              ]"
            >
              <span
                :class="[
          ...
          <div class="space-y-4">
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-ink/5 px-3 py-2">
                  <span class="font-medium text-ink">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-ink' : 'bg-ink/10'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-ink/5 px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_expiration ? 'bg-ink' : 'bg-ink/10'
              ]"
            >
              <span
                :class="[
              ...
              <button
                ...
                :class="[
                  ...
                  formData.expiration_preset === days
                    ? 'bg-ink text-paper'
                    : 'bg-paper text-muted border border-border hover:border-ink/40'
                ]"
              >
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border border-border hover:border-ink/40 hover:bg-ink/5 transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-muted" />
	            <span class="font-medium text-ink">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border border-border hover:border-ink/40 hover:bg-ink/5 transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-muted" />
	            <span class="font-medium text-ink">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max min-w-[380px] overflow-hidden rounded-md bg-surface shadow-medium border border-border duration-200"
      ...
      >
        ...
        <div class="border-b border-border p-2">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-sm border border-border bg-paper py-1.5 pl-8 pr-3 text-sm text-ink placeholder-subtle outline-none focus:border-ink/40 focus:ring-1 focus:ring-ink/15"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-subtle">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
