          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div class="relative w-full md:w-64">
              <Icon
                name="search"
                size="md"
                class="absolute left-3 top-1/2 -translate-y-1/2 text-subtle"
              />
              <input
                v-model="searchQuery"
                type="text"
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                >
                  ...
                  <button
                    ...
                    class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-ink hover:bg-ink/5 dark:text-muted"
                  >
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-border bg-surface py-1 shadow-lg"
                >
                  <button
        ...
        >
          <template #cell-email="{ value }">
            <div class="flex items-center gap-2">
              <div
                class="flex h-8 w-8 items-center justify-center rounded-full bg-ink/10"
              >
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                <Icon name="shield" size="xs" class="h-3.5 w-3.5 text-ink dark:text-ink" />
                <span class="font-medium text-ink dark:text-ink">{{ getUserGroups(row).exclusive.length }}</span>
                <span class="text-muted">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-border bg-surface py-1 text-xs shadow-xl"
                >
                  <div class="border-b border-border px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-subtle">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-ink transition-colors hover:bg-ink/5 hover:text-ink"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-subtle" />
                <span class="font-medium text-muted">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-subtle">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-ink/[0.02] px-2 py-1 text-xs text-subtle/50"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-ink underline decoration-dashed decoration-border underline-offset-4 transition-colors hover:text-ink"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-ink px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-ink"></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-success transition-colors hover:bg-success/[0.06]"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-muted">{{ t('admin.users.today') }}:</span>
            ...
            <div class="flex items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-success/[0.06]0' : 'bg-danger/[0.04]0'
                ]"
              ></span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-ink/5 hover:text-ink"
              >
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ...
                    : 'hover:bg-success/[0.06] hover:text-success'
                ]"
              >
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-ink/5 hover:text-ink dark:hover:text-white"
                :class="{ 'bg-ink/5 text-ink dark:text-white': activeMenuId === row.id }"
              >
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink hover:bg-ink/5 dark:text-muted"
              >
                <Icon name="key" size="sm" class="text-subtle" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink hover:bg-ink/5 dark:text-muted"
              >
                <Icon name="users" size="sm" class="text-subtle" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink hover:bg-ink/5 dark:text-muted"
              >
                <Icon name="dollar" size="sm" class="text-subtle" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-danger hover:bg-danger/[0.04]"
              >
